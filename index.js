...
async function startichigyou() {
    const { state, saveCreds } = await useMultiFileAuthState(`./${sessionName}`)

    const iikyy = WADefault({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['ichigyou','Safari','1.0.0'],
   ...
   // anticall auto block
    ...
    iikyy.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (mek.key.id.startsWith('BotzAku')) return
        m = smsg(iikyy, mek, store)
        require("./ichigyou")(iikyy, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    iikyy.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
               ...
               if (anu.action == 'add') {
                    iikyy.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*Hai Kak 👋🏻 @${num.split("@")[0]}, Welcome To Group ${metadata.subject}*\n\n_info selengkapnya baca deskripsi ya kak_\n\n*${namabot} by ${namaowner}*` })
                } else if (anu.action == 'remove') {
                    iikyy.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*Terima Kasih Kak @${num.split("@")[0]}, Sampai Bertemu Kembali di Grup ${metadata.subject}*` })
                } else if (anu.action == 'promote') {
                    iikyy.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*Selamat Ya @${num.split("@")[0]} Atas Kenaikan Jabatannya Di Grup ${metadata.subject} 🎉*` })
                } else if (anu.action == 'demote') {
                    iikyy.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*Nice Try @${num.split("@")[0]} Atas Penurunan Jabatannya Di Grup ${metadata.subject} 😔*` })
              }
            }
        } catch (err) {
...
try{
		if (connection === 'close') {
			...
			if (reason === DiconnectReason.badSession) {
				...
				console.log("connection Lost from Server, reiikyyecting...");
				startichigyou();
				...
				console.log("connection TimedOut, Reiconnecting...");
				startichigyou();
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
			...
			lolcatjs.fromString(`[connected] ` + JSON.stringify(iikyy.user, null, 2))
		}
    ...
    // Add Other
      ...
      * @param {*} options
      */
     iikyy.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
      let mime = '';
      let res = await axios.head(url)
      mime = res.headers['content-type']
      if (mime.split("/")[1] === "gif") {
     return iikyy.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options}, { quoted: quoted, ...options})
      }
      let type = mime.split("/")[0]+"Message"
      if(mime === "application/pdf"){
     return iikyy.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "image"){
     return iikyy.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options})
      }
      if(mime.split("/")[0] === "video"){
     return iikyy.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "audio"){
     return iikyy.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      }
      }
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        iikyy.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
        ...
        iikyy.sendMessage(jid, listMes, { quoted: quoted })
        }
     ...
     * @returns 
     */
        iikyy.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        iikyy.sendMessage(jid, templateMessage)
        }
     ...
     * @returns
     */
    iikyy.send5ButImg = async (jid , text = '' , footer = '', img, but = [], buff, options = {}) =>{
    iikyy.sendMessage(jid, { image: img, caption: text, footer: footer, templateButtons: but, ...options })
    }
       ...
       * @param {*} options
       */
      iikyy.send5ButLoc = async (jid , text = '' , footer = '', lok, but = [], options = {}) =>{
      let bb = await iikyy.reSize(lok, 300, 150)
      iikyy.sendMessage(jid, { location: { jpegThumbnail: bb }, caption: text, footer: footer, templateButtons: but, ...options })
      }
     ...
     * @returns
     */
    iikyy.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], buff, options = {}) =>{
    let lol = await iikyy.reSize(buf, 300, 150)
    iikyy.sendMessage(jid, { video: vid, jpegThumbnail: lol, caption: text, footer: footer, templateButtons: but, ...options })
    }
     ...
     * @returns
     */
    iikyy.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], buff, options = {}) =>{
    let ahh = await iikyy.reSize(buf, 300, 150)
    let a = [1,2]
    let b = a[Math.floor(Math.random() * a.length)]
    iikyy.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: ahh, templateButtons: but, ...options })
    }
     ...
     * @returns 
     */
    iikyy.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await iikyy.sendMessage(jid, { video: buffer, caption: caption, gifPlayback: gif, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    iikyy.sendAudio = async (jid, path, quoted = '', ptt = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await iikyy.sendMessage(jid, { audio: buffer, ptt: ptt, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    iikyy.sendTextWithMentions = async (jid, text, quoted, options = {}) => iikyy.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
...
iikyy.sendFakeLink = (jid, text, salam, pushname, quoted) => iikyy.sendMessage(jid, {
    ...
    iikyy.cMod = (jid, copy, text = '', sender = iikyy.user.id, options = {}) => {
		...
		copy.key.fromMe = sender === iikyy.user.id
